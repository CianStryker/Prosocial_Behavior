################################################################################
##### Figure A.12: Effect of Riot on Prosocial Behavior (IV; western Osh) ######
################################################################################
...
data_uzbek_west <- data_uzbek[data_uzbek$east_of_akbuura == 0, ]

# calculate distance
data_uzbek_west$distance <- 1-data_uzbek_west$apc_min_distance

# aggregate data
dataAgg <- aggregate(data_uzbek_west[,c("affected", "distance")], 
                     list(data_uzbek_west$id_psu),
                     mean)


##### Figure
### IV models ###
model11 <- lm(pd_in_scale ~ distance , data=data_uzbek_west)
model12 <- ivreg(pd_in_scale ~ affected | apc_min_distance, data = data_uzbek_west)
model21 <- lm(dg_in_scale ~ distance , data=data_uzbek_west)
model22 <- ivreg(dg_in_scale ~ affected | apc_min_distance , data = data_uzbek_west)
model31 <- lm(pd_out_scale ~ distance , data=data_uzbek_west)
model32 <- ivreg(pd_out_scale ~ affected  | apc_min_distance , data = data_uzbek_west)
model41 <- lm(dg_out_scale ~ distance , data=data_uzbek_west)
model42 <- ivreg(dg_out_scale ~ affected  | apc_min_distance , data = data_uzbek_west)
model51 <- lm(cooperation_index ~ distance , data=data_uzbek_west)
model52 <- ivreg(cooperation_index ~ affected  | apc_min_distance , data = data_uzbek_west)


dataAgg <- aggregate(data_uzbek_west[,c("apc_min_distance", "distance", "pd_in_scale", "dg_in_scale", "pd_out_scale", "dg_out_scale", "cooperation_index", "affected", "economy_index", "state_index", "social_cap_retro")], 
                     list(data_uzbek_west$id_psu),
                     mean)
names(dataAgg)[1] <- "psu"
...
figureA.12 <- ggplot(allModelFram, aes(shape=matrix_style)) + 
  ...
  coord_flip(ylim = c(-1.2,0.2)) + theme_bw() + 

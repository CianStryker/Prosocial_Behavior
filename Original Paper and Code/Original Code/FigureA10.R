################################################################
##### Figure A.10: Effect of Riot Destruction on Prosocial #####
##    Behavior (spatial lags)                              #####
################################################################
...
#Some cleaning
data_uzbek$social_cap_retro <- data_uzbek$leadership


# aggregate on psu level
dataAgg <- aggregate(data_uzbek[,c("pd_in_scale", "dg_in_scale", "pd_out_scale", "dg_out_scale", "cooperation_index", "affected", "economy_index", "state_index", "social_cap_retro", "access_index")], 
...
figureA.10 <- ggplot(allModelFram, aes(shape=matrix_style)) + 
